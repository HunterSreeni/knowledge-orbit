...
useSeoMeta({ title: 'Edit Post — Knowledge Orbit' })
...
// ── Load existing post ──────────────────────────────────
const { data: post, error: loadError } = await useAsyncData(
  `edit-${route.params.slug}`,
  () => store.fetchBySlug(route.params.slug as string, true)
)

if (!post.value) throw createError({ statusCode: 404, statusMessage: 'Post not found' })

// Guard: only the author can edit — block if author is unknown or current user doesn't match
const { data: { user: currentUser } } = await useSupabaseClient().auth.getUser()
if (!post.value.author?.id || !currentUser || post.value.author.id !== currentUser.id) {
  throw createError({ statusCode: 403, statusMessage: 'Forbidden' })
}

// ── Editor state (pre-filled) ───────────────────────────
const title = ref(post.value?.title ?? '')
const content = ref<Record<string, unknown> | null>(
  (post.value?.content as Record<string, unknown> | null) ?? null
)
const titleEl = ref<HTMLTextAreaElement | null>(null)

// ── Publish panel state ─────────────────────────────────
const showPublish = ref(false)
const summary = ref(post.value?.excerpt ?? '')
const coverUrl = ref(post.value?.cover_image_url ?? '')
const coverError = ref(false)
const tagInput = ref('')
const selectedTags = ref<{ id: number; name: string; slug: string }[]>(
  (post.value?.tags ?? []).map(t => ({ id: t.id, name: t.name, slug: t.slug }))
)
...
async function save(status: 'draft' | 'published') {
  ...
  try {
    ...
    await store.updatePost(post.value.id, {
      title: title.value,
      slug: slug.value,
    ...
    if (status === 'published' && post.value.status !== 'published') {
      await store.publishPost(post.value.id)
    }
    ...
    <header class="h-14 shrink-0 border-b border-default flex items-center justify-between px-4 bg-default z-10">
      <div class="flex items-center gap-2">
        <UButton
          ...
          aria-label="Back"
        />
        <span class="text-sm text-muted hidden sm:block">Edit Story</span>
      </div>
      <div class="flex items-center gap-2">
        <UButton
          variant="ghost"
      ...
      >
        <div class="w-full sm:max-w-2xl bg-default border border-default rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden">
          <div class="flex items-center justify-between px-6 py-4 border-b border-default">
            <div>
              <h2 class="font-semibold">Save changes</h2>
              <p class="text-xs text-muted mt-0.5">Update preview details, then save or publish</p>
            </div>
            <UButton
              icon="i-lucide-x"
              variant="ghost"
          ...
          <div class="p-6 grid sm:grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
            <div>
              ...
              <div class="rounded-xl border border-default overflow-hidden">
                <div class="h-36 bg-elevated flex items-center justify-center overflow-hidden shrink-0">
                  <img v-if="coverUrl && !coverError" :src="coverUrl" alt="Cover preview" class="w-full h-full object-cover" @error="coverError = true" />
            ...
            <div class="space-y-4">
              <div class="space-y-1.5">
                ...
                <UInput v-model="coverUrl" placeholder="https://i.imgur.com/xxxxx.jpg" class="w-full" @input="coverError = false" />
              ...
              <div class="space-y-1.5">
                ...
                <UTextarea v-model="summary" :rows="3" placeholder="What is this story about?" class="w-full" />
              </div>
